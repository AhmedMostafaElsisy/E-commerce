        ...
        child: BlocListener<ArchiveChatsCubit, ArchiveChatsStates>(
          listener: (archiveListCtx, archiveListState) {
            if (archiveListState is AddOrRemoveArchiveChatErrorState) {
              checkUserAuth(
                  context: context, errorType: archiveListState.error.type);
              showSnackBar(
                  context: context,
                  title: archiveListState.error.errorMassage!);
            } else if (archiveListState is AddOrRemoveArchiveChatSuccessState) {
          ...
          child: BlocListener<FavoriteChatsCubit, FavoriteChatsStates>(
            listener: (chatListCtx, chatListState) {
              if (chatListState is AddOrRemoveFavoriteChatErrorState) {
                ...
                showSnackBar(
                    context: context, title: chatListState.error.errorMassage!);
              ...
              builder: (chatListCtx, chatListState) {
                if (chatListState is FetchChatUsersLoadingState ||
                  ...
                  return EmptyScreen(
                    ...
                    description:
                        AppLocalizations.of(context)!.lblStartChatWithUserFirst,
                    withButton: false,
                  );
                } else {
                      ...
                      itemBuilder: (context, index) {
                        return InkWell(
                          onTap: () {
                            Navigator.of(context).pushNamed(
                                ...
                                arguments: RouteArgument(
                                    chatUserModel:
                                        chatUsersCubit.chatUserList[index]));
                          },
                          onLongPress: () {
                            pressAndHoldBottomSheet(
                                context: context,
                                isFavorite: chatUsersCubit
                                    .chatUserList[index].isFavorite!,
                                deleteChatClicked: () {
                                ...
                                archiveChatClicked: () {
                                  Navigator.pop(context);
                                  archiveChatsCubit.deleteOrAddUser(
                                    chatUserModel:
                                        chatUsersCubit.chatUserList[index],
                                    isAddToArchive: true,
                                  );
                          ...
                          child: UserInfoMassageItem(
                            isOnline:
                                chatUsersCubit.chatUserList[index].isOnline!,
